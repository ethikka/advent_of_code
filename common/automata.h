...
class automata {
  public:
    automata();
    automata(int x, int y, bool wraparound) { _x = x; _y = y; _wrap = wraparound; };
  private:
    int _x;
    int _y;
    bool _wrap;
    ...
    */
      ...
      for (int x = 0; x < _x; x++)
        for (int y = 0; y < _y; y++) {
          ...
          int key((x*_x)+y);
          for (int c = 0; c < 3; c++) {
            if (v[c] < 2) newstate[key][c] = 0;
            if (v[c] > 3) newstate[key][c] = 0;
            if (v[c] == 3 && !current_gen[key][c]) newstate[key][c] = 255;
            if ((v[c] == 2 || v[c] == 3) && current_gen[key][c]) newstate[key][c] = 255;
          }
        }
      for (int x = 0; x < _x; x++)
        for (int y = 0; y < _y; y++) {
    ...
    void place_pixels(int x, int y, std::vector<std::pair<int,int>> pixels, int r, int g, int b) {
      for (auto p: pixels) {
        int key = ((x+p.first)*_x)+(y+p.second);
        if (_wrap) 
          key = ((((x+p.first)+_x) %_x)*_x)+(((y+p.second)+_y)%_y);
  ...
  public: // Debugging functions
    void print_automata() {
      for (int x = 0; x < _x; x++) 
        for (int y = 0; y < _y; y++)
          printf("\33[%d;%dH\33[48;2;%d;%d;%dm  ", y+2, (x*2)+2, current_gen[(x*_x)+y][0], current_gen[(x*_x)+y][1], current_gen[(x*_x)+y][2]);
      printf("\33[39;49m\n");
      std::cout << std::flush;
    };
};
