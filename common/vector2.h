...
std::vector<vector2> lerp(vector2 min, vector2 max) {
  std::vector<vector2> res;
  int xmod = (min.x < max.x)? 1: -1;
  int ymod = (min.y < max.y)? 1: -1;
  for (int xx = min.x; xx != (max.x+xmod); xx += xmod)
  for (int yy = min.y; yy != (max.y+ymod); yy += ymod) res.push_back(vector2{xx, yy});
  return res;
};
...
//   std::vector<vector2> res;
//   for (int xx = min.x; xx <= max.x; xx++)
//   for (int yy = min.y; yy <= max.y; yy++) res.push_back(vector2{xx, yy});
//   return res;
// };
#endif