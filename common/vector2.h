...
template<class T>
bool operator<(const vector2<T>& a, const vector2<T>& b) { if(a.y == b.y) return a.x < b.x; return a.y < b.y; };
template<class T>
bool operator>(const vector2<T>& a, const vector2<T>& b) { if(a.y == b.y) return a.x > b.x; return a.y > b.y; };

namespace std {
  template<class T> struct std::hash<vector2<T>> { std::size_t operator()(const vector2<T>& k)const { return hash<long long>()(((long long)k.x)^(((long long)k.y)<<32)); }; };
}
...
std::vector<vector2<T>> lerp(vector2<T> min, vector2<T> max, vector2<T> ignore) {
  ...
  for (int yy = min.y; yy != (max.y+ymod); yy += ymod) if (ignore.x != xx || ignore.y != yy) res.push_back(vector2<T>{xx, yy});
  return res;
};
